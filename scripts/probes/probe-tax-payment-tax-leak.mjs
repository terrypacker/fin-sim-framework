...
function partTwo(opts) {
  const { scenario: file, index, stress } = opts;
...
partOne();
partTwo(parseFlags(process.argv.slice(2), {
  usage: 'node scripts/probes/probe-tax-payment-tax-leak.mjs [--stress]\n\n'
       + 'probe-tax-payment-tax-leak — does paying tax from a taxable account leak more tax?',
  stress:   { type: 'flag',   help: 'run the stressed arm' },
