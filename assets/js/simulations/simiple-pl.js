...
export class SimpleProfitLoss {
  ...
  reset() {
    ...
    this.assets.push(new Asset('item1', 1200, 200));
    this.assets.push(new Asset('item2', 10400, 400));
    this.assets.push(new Asset('item3', 20200, 200));
    this.assets.push(new Asset('item4', 9200, 1200));
    ...
    this.sim = new Simulation(this.simStart, {
  ...
  initFlow() {
    ...
    this.sim.reducers.register('ADD_CASH', (state, action, date) => {
      this.accountService.transaction(state.savingsAccount, action.amount, date);
      return {
        ...state
      };
    ...
    if (this.debug) {
      this.sim.bus.subscribe('DEBUG_ACTION', (event) => {
        console.log(
