 ...
 */
export class MapFilterMultiSelect extends BaseComponent {
  constructor({
    parent,
    container,
    selectedItems = [],
    ...
    // internal state
    this._query = {
      query: this._defaultCondition ? this._defaultCondition : '',
  ...
  _onClick(e) {
    ...
    if (this._selectedMap.has(id)) {
      ...
      this._onToggle(selectedItem, false, [...this._selectedMap.values()]);
    } else {
      this._selectedMap.set(id, selectedItem);
      this._onToggle(selectedItem, true, [...this._selectedMap.values()]);
    }
  ...
  async _fetchPage(reset = false) {
    ...
    const { items, total } = await this._queryApi.search(this._query);
