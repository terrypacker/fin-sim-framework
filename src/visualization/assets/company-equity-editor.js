...
/**
 * CompanyEquityEditor — renders the company-equity edit form from
 * tpl-company-equity-editor into a given container.
 *
 * Communicates outward via callbacks:
 *   onSave(data)    — user clicked Save
 *   onDelete(id)    — user clicked Delete
 */
export class CompanyEquityEditor extends BaseComponent {
  /**
   * @param {{
  ...
  render() {
    const el     = this._getTemplate('tpl-company-equity-editor');
    ...
    el.querySelector('[data-id="appreciationRate"]').value = this._node?.appreciationRate ?? 0.08;
  ...
  /** Route param-backed company-equity fields through their param (design/32). */
  _bindParamLinks(el) {
    this._linkedFields = new Set();
    const stateKey = this._node?.stateKey;
    if (!stateKey || !this._links) return;

    const param = this._links.getParamFor('companyEquity', stateKey, 'plannedSaleYear');
    if (!param) return;
