...
/**
 * After PATH-4 loadDefaults(), apply saved field edits from the config so
 * that user changes to persons, accounts, real properties, and collectibles
 * survive a Save → Load cycle.
 *
 * loadDefaults() creates domain objects from code defaults, ignoring anything
 * the user may have changed through the UI editor and persisted via onSave.
 * This function re-applies those saved values by matching live objects to
 * serialized records via stable keys (person.id, account.stateKey,
 * property.stateKey, collectible.stateKey).
 *
 * @param {object} config   - saved scenario config (activeConfig)
 * @param {object} registry - ServiceRegistry instance
 */
function _applyLoadDefaultsOverrides(config, registry) {
  if (!config) return;

  // Persons — matched by id ('primary' / 'spouse' are stable across reloads).
  if (config.persons?.length) {
    ...
    for (const d of config.persons) {
      ...
      if (live) {
        const { id, __type, ...changes } = d;
        svc.updatePerson(live, changes);
      }
    }
  }

  // Accounts — matched by stateKey; id is regenerated by createAccount() each load.
  // initialValue in the serialized record maps to balance on the live object.
  ...
  // Real properties — matched by stateKey.
  if (config.realProperties?.length) {
    ...
    for (const d of config.realProperties) {
      ...
      if (live) {
        const { id, __type, ...changes } = d;
        svc.updateProperty(live, changes);
      }
    }
  }

  // Collectibles — matched by stateKey.
  if (config.collectibles?.length) {
    ...
    for (const d of config.collectibles) {
      ...
      if (live) {
        const { id, __type, ...changes } = d;
        svc.updateCollectible(live, changes);
      }
    }
  }
}
...
export class BaseApp extends BaseComponent {
  ...
  initScenario() {
    ...
    if (hasSerializedConfig) {
      ...
      // loadDefaults() rebuilds from code defaults; apply any saved field edits
      // that were persisted by onSave, so user changes survive a Load cycle.
      _applyLoadDefaultsOverrides(activeConfig, registry);
    }
