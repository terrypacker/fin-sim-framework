    ...
    paramSchema: () => [
      {
        ...
        description: 'The pool GRAPH: { pools: [...], flows: [...] } (design 97 Part II). A pool is '
          ...
          + 'A clause can also carry an `id` (letters, digits, `_`, `-`), which is its ADDRESS: it '
          + 'changes nothing about the run and makes that one clause SEARCHABLE, generating the '
          + '`gate.<id>.threshold` and `gate.<id>.dwell` levers the optimizer and the MC grid can '
          + 'sweep. Without one a clause is positional — the OR # renumbers on every edit — so it '
          + 'cannot be addressed and no axis for it exists. Ids are unique within a graph; the same '
          + 'id in two shapes is the same clause, and one sweep moves both. '
