...
panels: []
params: [liquidityShapes, liquidityGraphSchedule]
actions: []
tools: []
design: [109-time-varying-pool-shapes.md, 97-liquidity-pools-and-drawdown-sequence.md]
sources: []
stamps:
...
combination live in some particular year can break rules every individual timeline
respects, with nothing to catch it — there would be no moment at which the whole thing was
checked. Naming the structure keeps every version complete and separately validated when
...
Renaming between versions therefore retires one bucket and starts another. Doing that
deliberately is a decision; doing it by accident is the trap.
