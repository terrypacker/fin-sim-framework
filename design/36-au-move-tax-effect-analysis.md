...
- **What's wrong**: the AU capital gain is computed from the asset's **original (US) cost basis**, not the market value when the person became an AU resident.
  ...
  - US-brokerage gain reaching AU: computed in `AccountService._drawPenaltyFree` (BROKERAGE branch) from holding `earningsBasis`/`costBasis`; the `STOCK_WITHDRAWAL_TAX` AU branch (`us-tax-module-2026.js:179`) then books that same `gain` to `auCapitalGainsYTD`.
  - AU house gain: `AU_HOUSE_SALE_TAX` (`au-tax-module-2026.js:209`) — same original-basis issue.
- **Correct behaviour (ITAA 1997 s855-45)**: a person becoming an AU resident is taken to have **acquired their non-Taxable-Australian-Property assets at market value on the residency date**. So the **AU** cost base resets to market-value-at-residency-change, while the **US** cost base (US citizen) stays original. This is genuinely a **dual cost base** — the two gains differ and are relieved against each other via FTC.
- **Implementation sketch** (bigger than 12.1):
  1. At `CHANGE_RESIDENCY`, snapshot a per-holding AU cost base — extend `recordResidencyChange` to also stamp `holding.auCostBasis = holding.marketValue` (today it only stamps the account-level `balanceAtResidencyChange`). Pre-residency-acquired lots get the reset; lots acquired *after* the move keep their actual cost.
  2. Where a gain is realised for an AU resident, compute **two** gains: `usGain = proceeds − usCostBasis` and `auGain = proceeds − (auCostBasis ?? usCostBasis)`. Carry both on the `*_WITHDRAWAL_TAX` / `*_HOUSE_SALE_TAX` action (e.g. `gain` for US, `auGain` for AU).
  3. Route in the tax modules: `usCapitalGainsYTD += usGain`; for AU residents `auCapitalGainsYTD += auGain` (then the existing 50% discount + FTC apply). Touch points: `au-tax-module-2026.js` `AU_STOCK_WITHDRAWAL_TAX`/`AU_HOUSE_SALE_TAX`, and `us-tax-module-2026.js:179` `STOCK_WITHDRAWAL_TAX` AU branch.
- **Tests**: new EVT case — asset with original basis B, market value M at residency change, sold post-move for P: assert `usCapitalGainsYTD += (P−B)` but `auCapitalGainsYTD += (P−M)`. Cover an asset acquired *after* the move (no reset → both use actual basis).
- **Verify end-to-end**: in §7's tally, `STOCK_WITHDRAWAL_TAX:AU`/`AU_STOCK_WITHDRAWAL_TAX` gain base (move ≈ $1.95M today) should **drop** by the pre-2031 appreciation; re-run §3 — the move advantage should widen beyond +28%.
