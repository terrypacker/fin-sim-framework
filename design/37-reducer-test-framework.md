...
| `MpcDecisionScheduleReducer` (design 81 §5) | ✅ `mpc-run-schedule.test.mjs` — MRS-6 (a period before every decision adds no field, so an unselected bag is byte-identical), MRS-7 (crossing a row's date stamps `mpcDecisionApplied` once and the next undeciding period re-stamps nothing), MRS-8 (a lever with no `applyAt` warns loudly rather than playing the run back short); `mpc-run-absent.test.mjs` — MRA-1/2/3 (the registration gate is the SELECTION, not the bag, and the OFF switch keeps it), MRA-4 (a played run ≡ the from-scratch date-keyed plan, state for state), MRA-5 (the decision reaches `ExplicitBandsSpendingReducer`, so MRA-4 is not passing vacuously). I1-pure. |
