...
- **Simplex / allocation levers → residual-encode for sampling, native equality for the QP.** The design 61 holding-allocation lever adds a *new lever type*: a set of `K` weights (EQUITY/BOND/CASH/GOLD) whose **applied value is the normalized mix** `w_i / Σ w_j`. Encoded naively as `K` free box coordinates, the sim response is **scale-invariant** — `w` and `2w` map to the *same* mix — so the objective has a **flat, non-identifiable ray** along the all-ones direction. That is the same pathology as the design-58 phantom drawdown dims: a direction the surface can't fit and the trust region can't shrink, wasting rollouts and reporting an arbitrary radius. **Two coordinated fixes:** (a) for the **CEM/coverage** pass and the fitted surface, search the **simplex intrinsically** — `K−1` free coordinates with the last allocation as the residual `1 − Σ`, or fix a reference sleeve to weight 1 — which removes the degeneracy (the surface is then full-rank in the reduced coordinates); (b) for the **QP step**, carry the simplex **natively** as a linear equality `Σw = 1` plus the box `w ≥ 0` (the §7 QP already admits linear constraints), rather than folding normalization into the objective. **Pivot seam:** an `leverGeometry` interface (`box` default, `simplex` for allocation) that the surface fitter and QP builder both consult, so a future ratio/share lever reuses the same reduction. See `design/61-holding-allocation-lever.md` §4-A / OQ1.
...
- [ ] Re-measure once design 61's allocation lever (`allocWeight::*`, 3–4 continuous
      simplex dims + interactions) is enabled online — another dimension-growth
      motivator for the surrogate. **Note:** design 46 does **not** block design 61 —
      61 Phases 1–4 ride existing CEM; only 61's Phase-5 *performance* (many levers
      online together) is a soft coupling. See `design/61-holding-allocation-lever.md`
      §11.1, and the simplex `leverGeometry` pivot seam in §6 above.
