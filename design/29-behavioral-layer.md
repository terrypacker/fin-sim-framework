...
**BUILT, with one refinement (see `docs/economic-shocks/README.md` §1 "Tags, and the stress window").** Tags are read **per LEG** (`leg.tags ?? shock.tags`), not shock-wide, because a leg is a regime with its own window and a tag does not decay with the recovery factor — it is present or absent while its regime lives. Stamping one shock-level tag onto every leg would have made the stress window the LONGEST leg: the GFC's easing leg runs 84 months, so `ContributionSuspension` would have halted contributions for the whole easing cycle. Each tagged preset therefore carries a dedicated `stress` leg with no rate adjustments, whose `durationMonths` is the measured peak→trough and whose only job is to state the window. Until this landed, no library preset carried tags at all and every strategy in §3.1/§3.2/§3.3/§3.5/§3.7 was configurable and inert.

**Intensity is a scalar, not a graded tag** (design 21 §24). Tags stayed a KIND channel; `regime.severity` — the shock's measured trough depth, stamped onto every leg by `EconomicShockHandler` — is the magnitude, and strategies gate on a threshold of their own (`contributionSuspensionMinSeverity`, `cashBucketDrawdownMinSeverity`, both 0.25). `STRESS_LOW/MED/HIGH` was rejected because `applySeverity` rescales a shock and leaves its tags untouched, so a declared level cannot follow an MC sweep — and because `REGIME_TARGET_PRIORITY`/`regimeTargets` already key an authored map on tag names, which a graded vocabulary multiplies. This also supersedes the `panicSellSeverity` regime property specified in §3.1 below, which was never built: `regime.severity` is that field.
