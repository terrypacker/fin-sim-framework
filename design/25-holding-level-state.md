...
Tests assert this invariant in three places: (a) a generic `holdings-invariant.test.mjs` walks every account after every reducer step and asserts the sum, on one scenario over a few years with a flat ±$1.00 tolerance; (b) round-trip serialization tests assert the invariant holds before and after `toJSON`/`fromJSON`; (c) since design 106 §4b, **`findOutOfSync` checks it on the end state of every GOLDEN** — long runs, every scenario, a per-lot tolerance of `0.01 + 0.005 × lots`, and an empty holdings array is checked rather than skipped. (c) is what caught a brokerage carrying A$67,428 against zero lots for the life of its fixture: the goldens pin values, and until then nothing pinned the relationship between them. Known violations are waived, with what each is worth, in `tests/helpers/golden-invariant-waivers.js`.
