...
- **Phase 3 — state move + optimization. ✅ landed 2026-06-19 (Jan-1 first cut).** `CHANGE_STATE_RESIDENCY` event + `ChangeStateResidencyHandler` + `ChangeStateResidencyApplyReducer` (sets `residencyState = destination` on every person at `PRE_PROCESS`), wired in `US_STATE_TAX` (`paramSchema` + `schedules`/`handlers`/`reducers`, registered `CHANGE_STATE_RESIDENCY_APPLY` action type). `stateMoveYear` (opt INTEGER axis + dynamic NORMAL MC sweep when set) and `stateMoveDestination` (opt ENUM categorical axis; opt-only — no categorical MC distribution exists, mirrors `moveYear`/`startingResidency`). Moves are **pinned to Jan 1** (§9.1 recommended first cut): the destination state taxes the whole calendar year, so no apportionment and no settle-side change — the Dec-31 settle resolves the active state at runtime. Tests: `change-state-residency.test.mjs` (unit) + `EVT-STATE-MOVE-1/2` in `evt-state-tax.test.mjs`. **Full part-year apportionment remains deferred** to the phase that generalizes AU part-year residency (the `stateMovePeriodMs`-marker / split-at-settle path in §9.1).
