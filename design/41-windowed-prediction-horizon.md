...
**Status**: Proposed (draft 2026-06-28) — decisions locked (§8 D1, D2); ready to implement Phase 1.
...
- **Phase 1 (this design).** `horizonYears` + `_scoreEnd()` gate in `OptimizationProblem`; `windowable` tag + `objectiveIsWindowable`; controller `setHorizonYears`; cockpit `Horizon` field with the non-windowable disable/hint; the identity + fix + myopia-guard tests. Delivers the Roth flagship: `MAX_AFTER_TAX_NET_WORTH` on a window recommends a real interior conversion.
