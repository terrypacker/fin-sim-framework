...
**Status**: Implemented (2026-06-28). Phases 1–3 landed; see §5 for per-phase notes.
...
- **Phase 1 — stop the holdings corruption (contained, high-value).** ✅ *Done.* Removed the destructive single-holding special case in `rescaleHoldingsToBalance` so a lone holding flows through the proportional scaling (`costBasis *= target / Σmv`); `costBasis = target` now happens **only** in the `curSum<=0` (marketValue == 0) branch. Rebuilds/param-edits no longer wipe unrealized gain. Tests: `holding-balance-sync.test.mjs` invariant-3 cases (single-lot gain preserved, per-lot ratio incl. a loss lot).
- **Phase 2 — basis-correct drawdowns.** ✅ *Done, via scoped Route B.* Investigation showed the drift came from one gap: `_drawPenaltyFree`'s **eligible** branch updated the ledger only for `BROKERAGE`, so an age-eligible super/IRA/401k drawn by the engine moved `balance` but not the ledger (the trigger). Added `AccountService.reduceLedgerForWithdrawal(account, amount)` (per-type rule: Roth/IRA contributions-first, 401k earnings-first, super/default proportional; **unrounded** so the reduction ties to the balance move) and call it from that branch for ledger-bearing non-brokerage accounts. *Not* Route A (FIFO carries no contribution/earnings tax character) and *not* inside `transaction()` (would double-count the type reducers, which remain the authority). Tests: `account-service.test.mjs` (per-type + eligible-super `replenishSavings` ledger tie). **Known follow-up (out of scope):** that branch still emits no withdrawal-tax action for eligible super/IRA/401k, and the cross-border cash sweep emits no transfer action — both written up in `design/44-cross-border-drawdown-actions.md`.
- **Phase 3 — reconcile / guard.** ✅ *Done.* `reconcileLedgerToBalance(account)` (in `investment-account.js`) clamps an **over-stated** ledger to balance preserving the earnings fraction (the corruption signature; an under-stated ledger = benign "earnings not yet recorded" is left alone), wired into `ScenarioSerializer._makeAccount` beside the holdings auto-heal so it runs before any consumer reads the ledger. Property harness `basis-invariants.test.mjs` drives 400+ randomized contribution/withdrawal/market-move/rebuild ops (super proportional + Roth contributions-first) asserting all four invariants after every op.
- **Decision — keep the account field (§6).** ✅ Kept. Did *not* remove `contributionBasis`/`earningsBasis`.
