...
**Status**: Phases 1, 3 & the Phase-2 AU-engine path implemented + browser-verified (2026-06-28); decisions locked (§7 D1, D2). Deferred: §408A recapture, primary-residence cap-gains exclusion, and the cross-border AU-brokerage US-LTCG+FTC reconciliation.
...
- **Phase 2 (fidelity).** *AU-side engine path — [x] IMPLEMENTED.* Two model-faithful corrections (extending the Option-C `liquidationRateProvider` + the SUPER branch of `computeAfterTaxValue`):
  - **Super basis-split (applies under both rate methods).** AU super is no longer a flat-rate hit on the whole balance. Post-preservation-age the **contribution basis comes out tax-free** and only **earnings** are taxed — and the model taxes super earnings as **US ordinary income** (`SUPER_WITHDRAWAL_EARNINGS_TAX → usOrdinaryIncomeYTD`, no AU tax). So `value = (balance − earningsBasis) + earningsBasis·(1 − r)`, and under Option C `r` is the **US ordinary** effective rate (super routes through `computeUsTax`, alongside IRA/401k). When `earningsBasis` is absent/non-finite, the whole balance is taxed (back-compat). *Robustness:* uses `balance − earningsBasis` for the contribution portion (not the `contributionBasis` field, which can drift out of sync with balance — observed live).
  - **AU brokerage CGT via the engine.** `au-stock`/`au-fixed-income` cap gains go through **`computeAuTax`** (the resident **50% CGT discount** + AU brackets + Medicare), not a flat rate. *Documented approximation:* the runtime adds AU-stock gains to **both** `usCapitalGainsYTD` and `auCapitalGainsYTD` (cross-border, reconciled by FTC); the metric captures the **AU side only** — the US-LTCG + FTC reconciliation is the remaining refinement.
  - All engine deltas now read **`netLiability`** (the true marginal total tax: brackets, Medicare, CGT discount, FTC interaction). Tests: `after-tax.test.mjs` (+3: super basis-split par-on-contribution, super earnings via US engine, AU CGT via `computeAuTax`). Full suite green (2961 unit, 798 viz). **Browser-verified:** super now values at **par** when it is all contribution basis (was flat-15% taxed) — model-faithful. *Open for the user's validation:* live `superAccount` shows `contributionBasis` (180k) > `balance` (39k) with `earningsBasis` 0 — a model basis-tracking inconsistency the metric is robust to but worth resolving.
  - *Still deferred:* §408A 5-year recapture and primary-residence cap-gains exclusion (Q5); residency-aware Option B (skipped — Option C covers fidelity).
- **Phase 3 (the real-world calculation). [x] IMPLEMENTED.** `liquidationRateProvider` (`after-tax.js`) — the Option C waterfall. **Revision to the original plan:** rather than a *new* `BaseTaxModule.taxOnHypotheticalOrdinaryIncome(...)`, it reuses the engine that already exists — **`TaxSettleService.computeUsTax(state)`**, the same inflation-adjusted, year-resolved (`state.currentPeriods`/`inflationAccumulator`) computation the sim settles with. The effective rate is the **delta**: `(computeUsTax({…state, usOrdinaryIncomeYTD: ytd+amount}).ordinaryTax − computeUsTax(state).ordinaryTax) / amount`, i.e. the liquidation stacked on the realized income at the valuation date; cap-gains likewise via `capitalGainsTax`. Selected by a new `afterTaxRateMethod` scenario param (`configured` = Option A default · `liquidation` = Option C); drops into the Phase-1 `rateProvider` seam with **no metric/objective change** (D1). **Scope:** US pre-tax (IRA/401k → US ordinary, taxed regardless of residency, §2.3) and US brokerage (LTCG); **AU/super** get their own engine path in Phase 2 (below) — super via basis-split + US-ordinary on earnings, AU brokerage via `computeAuTax`. Any error/non-finite result also falls back, so the metric never throws. *Approximation (documented):* per-account marginal stack on realized income, not a joint multi-account liquidation. Tests: `after-tax.test.mjs` (+6: engine effective rate in bracket-fill range, progressive, LTCG, AU fallback, graceful, end-to-end method-switch). Full suite green (2959 unit, 798 viz). **Browser-verified** (2026-06-28): on the live scenario at an 8-yr window, Option C and Option A both peak at the same conversion (~\$100k) but Option C values the terminal pre-tax pile at the real bracket-stacked rate (after-tax liq \$1.879M vs the flat-22% \$1.850M) — the more accurate pricing.
