...
test('EVT-REGIME-STACK-1: two overlapping shocks stack additively on effectiveGrowthRates', () => {
  ...
  // Per-account member key: EQUITY_US = base 0.07 − 0.02 − 0.01 = 0.04
  ...
  assert.ok(
    ...
    `Expected EQUITY_US ~0.04 (stacked), got ${effectiveRate}`
  );
...
test('EVT-REGIME-STACK-2: expired V-curve regime is dropped from state.activeRegimes', () => {
  ...
  assert.ok(
    ...
    `EQUITY_US should return to base 0.07 after expiry, got ${effectiveRate}`
  );
});
